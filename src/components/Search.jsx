...
function HighlightQuery({ text, query }) {
  return (
    <Highlighter
      highlightClassName="underline bg-transparent text-vest-500"
      searchWords={[query]}
      autoEscape={true}
  ...
  return (
    ...
    >
      <div
        id={`${id}-title`}
        aria-hidden="true"
        className="text-sm font-medium text-zinc-900 group-aria-selected:text-vest-500 dark:text-white"
      >
    ...
    <div className="group relative flex h-12">
      ...
      {autocompleteState.status === 'stalled' && (
        <div className="absolute inset-y-0 right-3 flex items-center">
          <LoadingIcon className="h-5 w-5 animate-spin stroke-zinc-200 text-zinc-900 dark:stroke-zinc-800 dark:text-vest-400" />
        </div>
      )}
    </div>
  )
